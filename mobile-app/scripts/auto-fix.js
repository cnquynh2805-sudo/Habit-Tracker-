...
function fixInlineStyles() {
  ...
  c = c.replace(
    /style=\{\{\s*flexDirection:\s*'row',\s*alignItems:\s*'center',\s*flex:\s*1,\s*justifyContent:\s*'flex-start'\s*\}\}/g,
    "style={styles.headerLeftContainer}",
  );
  c = c.replace(
    /style=\{\{\s*position:\s*'relative',\s*marginLeft:\s*8\s*\}\}/g,
    "style={styles.langMenuWrapper}",
  );
  c = c.replace(
    /style=\{\{\s*padding:\s*8,\s*justifyContent:\s*'center',\s*alignItems:\s*'center'\s*\}\}/g,
    "style={styles.headerIconButton}",
  );
  c = c.replace(
    /style=\{\{\s*position:\s*'relative',\s*marginRight:\s*12\s*\}\}/g,
    "style={styles.themeMenuWrapper}",
  );
  c = c.replace(
    /style=\{\{\s*position:\s*'relative'\s*\}\}/g,
...
function fixA11y(file) {
  ...
  c = c.replace(
    /accessible=\{true\}\s+accessible=\{true\}/g,
    "accessible={true}",
  );
  c = c.replace(
    /accessibilityRole="button"\s+accessibilityRole="button"/g,
    'accessibilityRole="button"',
  );
  c = c.replace(
    /accessibilityLabel="Interactive element"\s+accessibilityLabel="[^"]*"/g,
