...
function reportViolation(file, line, rule, message) {
  totalViolations++;
  reports.push(`\x1b[31m[Violation]\x1b[0m ${path.basename(file)}:${line} -> ${rule}: ${message}`);
}
  ...
  traverse(ast, {
    ...
    JSXOpeningElement(pathNode) {
      ...
      if (elementName === 'TouchableOpacity' || elementName === 'Pressable') {
        const hasAccessible = pathNode.node.attributes.some(attr => attr.name && attr.name.name === 'accessible');
        const hasA11yRole = pathNode.node.attributes.some(attr => attr.name && attr.name.name === 'accessibilityRole');
        const hasA11yLabel = pathNode.node.attributes.some(attr => attr.name && attr.name.name === 'accessibilityLabel');
      ...
      if (
        ...
        pathNode.traverse({
          ObjectProperty(stylePropPath) {
            const propName = stylePropPath.node.key.name || stylePropPath.node.key.value;
            const propValue = stylePropPath.node.value;
            
            if (['width', 'height', 'maxWidth', 'minWidth', 'maxHeight', 'minHeight'].includes(propName)) {
...
if (totalViolations === 0) {
  console.log(`\x1b[32mSuccess! 0 UI Guideline violations found across ${files.length} files.\x1b[0m`);
  process.exit(0);
} else {
